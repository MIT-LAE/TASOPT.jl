function LTO(name, NPSS::Base.Process, ifirst, pare; fileout = stdout)
    ...
    mdotf, EINOx1, FAR, Tt3, OPR,
    Wc3, Tt41, EGT = NPSS_TFsysOD(NPSS, 0.0, 0.0,
    Foo, 0.0, ifirst, parg, parpt, pare, ip)
    ...
    "#", "TS", "Fn[kN]", "̇mf[kg/s]", "EI(NOₓ)[g/kg]", "NOₓ[g/s]", "deNOₓ[%]", "JetAeqEI(NOₓ)" ))
        ...
        for (i,TS) in enumerate(LTOpoints)
            ...
            mdotf, EINOx1, FAR, Tt3, OPR, Wc3, Tt41, EGT = NPSS_TFsysOD(NPSS, 0.0, 0.0, Foo*TS, 0.0, ifirst, parg, parpt, pare, ip)
            ifirst  = false

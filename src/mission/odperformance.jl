function odperf!(ac, W, FL, Ldebug)
    ...
    for   i = 1:N
        ...
        for  iterg = 1:itergmax
            ...
            itrim = 1
            balance(ac.pari, ac.parg, view(ac.parad, :, ip), ac.fuselage, ac.wing, ac.htail, ac.vtail, rfuel, rpay, ξpay, itrim)
            # Calculate Drag
            if (i == 1)
                icdfun = 0
            else 
            ...
            cdsum!(ac.parg, view(ac.parad, :, ip), view(ac.pared, :, ip), ac.wing, ac.htail, ac.vtail, icdfun)

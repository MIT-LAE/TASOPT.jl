...
function stickfig(ac::aircraft; plot_obj = nothing, label_fs = 16, 
    ...
    xi .= tanL * (yi .- bo / 2) .- 0.4 .* ci .+ parg[igxwbox] .- 1.0
    xlocations = vec(hcat([xi, xi, xi .+ lnace, xi .+ lnace, xi]...))
    ...
    if annotate_text
        annotate!(plot_obj, -18, 21, text("PFEI = $(round(parm[imPFEI], digits = 4))\n" *
            ...
            "c₀ = $(round(parg[igco], digits=1)) m\n" *
            "Λ = $(round(parg[igsweep], digits=1))°\n" *
            "Rfuse = $(fuselage.layout.radius) m\n" *
            "L/D = $(round(para[iaCL][1] / para[iaCD][1], digits=2))",
    ...
    # Span annotations
    find_aerodrome_code(bmax) = (["C", "III"], bmax)  # Replace with actual function

...
include("../engine/PMSM.jl")
include("../engine/PMSM.inc")
include("../engine/NPSS_functions.jl")
...
function run_wsize(iter, initwgt, is_Ldebug, is_printiter, is_saveOD)
    parpt[ipt_time_NPSS] = 0.0
    parpt[ipt_calls_NPSS] = 0

    wsize(pari, parg, parm, view(para,:,:,1), view(pare, :,:,1),
                    iter, 0.5, 0.9, 0.5, initwgt, 1, 1, is_Ldebug, is_printiter, is_saveOD)

    cost = CostEst(pari, parg, pare, parm, parpt, 1000)

    return cost
end

@testset "wsize test" begin
    ...
    cost = run_wsize(iter, initwgt, is_Ldebug, is_printiter, is_saveOD)

    @test cost ≈ 8.847524437032087e7 rtol = 1e-8
